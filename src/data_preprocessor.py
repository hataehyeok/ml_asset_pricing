...
def create_return(firm_predictors, risk_free, horizon):
    '''
    ...
    firm_predictors['Return'] = firm_predictors.groupby('permno')['LogPrice'].diff(horizon)
    ...
    firm_predictors['Return'] =  firm_predictors['Return'].shift(-horizon)

    return firm_predictors

def shift_consensus(firm_predictors, firm_info, horizon):
    '''
        Shift consensus columns to solve consensus prediction problem
    '''
    analyst_col = list(firm_info[firm_info['Cat.Data']=='Analyst']['Acronym'].values)
    firm_predictors_analyst = firm_predictors.groupby('permno')[analyst_col].shift(-horizon)
